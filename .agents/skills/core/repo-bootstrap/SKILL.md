...
- `scripts/generate/init-generated-project.sh`
- `scripts/generate/init-harness-task.sh` only if the user also wants the first task pack
...
4. If the repository is `missing` and the user wants the standard layout, run `bash scripts/generate/init-generated-project.sh --root <repo-root>`.
