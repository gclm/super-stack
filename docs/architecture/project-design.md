...
### 4.4 模板与生成骨架：`templates/generated-project/`

这一层主要服务两件事：

1. 生成 target project 的 `docs/ + harness/` 初始骨架
2. 提供可复用的 validation / reference 起始文档

这层的核心价值是：

- 把“项目状态”和“验证结果”从对话转成文件，并让新项目按同一信息架构起步
...
- 底层方案：宿主侧 browser MCP / browser plugin
- Codex 当前主链路：`chrome-devtools-mcp`
- 安装方式：由宿主自行配置，不再由 `super-stack` 安装 `agent-browser` wrapper
...
- [浏览器技术选型记录](decisions/browser-technology-options.md)
...
- 跨轮恢复 `harness/state.md`（当前态）和 `harness/history.md`（持久记录）
- 降低上下文漂移
