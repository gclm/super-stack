...
1. 当前文档已经解决了什么，缺什么
2. 自动更新机制应该怎样接入现有 `skills / protocols / docs + harness / source repo` 体系
...
1. 写入 source repo
2. 运行文本级自检
3. 更新 `harness/state.md`（当前态），追加 `harness/history.md`（持久记录）
...
结合公众号文章与 `stellarlinkco/skills`，我建议把当前状态模型在原则上再升级一层：

- `harness/state.md` 不只是“阶段状态记录”
- 在长任务场景里，`harness/tasks/<task-id>/` 还应承担“可恢复上下文”的角色

这不意味着把 `harness/` 变成重型 project database，而是意味着：

- 对明显跨 session 的复杂任务，`docs/overview/roadmap.md`、提案文档与 `harness/state.md`（当前态）、`harness/history.md`（持久记录）应更明确记录当前阶段、下一步、验证口径、未决决策
- 自动复盘产物与 evolution ledger 不放进 `harness/tasks/`，但它们应与 `docs/ + harness/` 形成稳定互补
- 当 retrospective 发现“过早完成、缺少进度产物、恢复失败”这类问题时，应回头升级 `docs/ + harness/` 约定，而不是只改回答措辞
...
- `harness/state.md` 回写当前态，`harness/history.md` 追加持久记录
- evolution ledger 追加
