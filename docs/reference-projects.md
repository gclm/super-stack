...
|----------|------------|------------------------|
| GSD | 多宿主工作流思路、context engineering、spec 驱动意识 | `AGENTS.md`、技能分层、全局优先策略 |
...
| bdarbaz/claude-stack-plugin | 共享核心 + 宿主适配层 | `.claude/`、`.codex/` 只做薄适配 |
...
这轮之后，浏览器能力已经不再只是“概念存在”：

- 本地 `agent-browser` 已安装并可用
- 稳定入口 `super-stack-browser` 已建立
- `check-browser-capability.sh` 已能识别主 provider 和 fallback provider
...
- 保留 `agent-browser` 作为唯一浏览器方案
- 用 `super-stack-browser` 作为稳定入口
- 后续浏览器优化只围绕这条主链路继续做
