...
- current focus: source repo 已完成可见路径收敛（`skills/`、`codex/`、`claude/`）；当前重点是继续收紧 runtime，只保留 host adapter、运行态必需 `hooks + workflow + 受管 check 子集` 与其最小依赖，避免把 source 运维脚本整包带进 runtime。
- active constraints:
...
- active decisions:
  ...
  - `validate-skills.py` 通过 manifest 驱动的路径语义规则区分 source/runtime 引用，并扫描 `SKILL.md` 与 `references/*.md` 中的受管路径写法。
  ...
  - runtime `scripts/` 只保留运行态必需子集：`hooks/`、`workflow/`、`scripts/lib/common.sh`，以及白名单 `scripts/check/check-browser-capability.sh`、`scripts/check/check-codex-runtime.sh`、`scripts/check/validate-skills.py`；`install`、`smoke`、`test`、`release` 与 `scripts/lib/install-state.sh` 不进入 runtime。
- next actions:
