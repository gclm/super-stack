...
check_browser_capability() {
  ...
  case "$(printf '%s' "$result" | sed -n '1p')" in
    ACTIVE_LOCAL)
      ok "Claude browser capability: active local provider ($(printf '%s' "$result" | tr '\n' ' ' | sed 's/  */ /g'))"
      ;;
    ACTIVE_MCP)
      ok "Claude browser capability: active MCP provider ($(printf '%s' "$result" | tr '\n' ' ' | sed 's/  */ /g'))"
      ;;
    ACTIVE_PLUGIN)
      ok "Claude browser capability: active browser plugin ($(printf '%s' "$result" | tr '\n' ' ' | sed 's/  */ /g'))"
      ;;
    INSTALLED_ONLY)
      warn "Claude browser capability: browser plugin installed but not enabled ($(printf '%s' "$result" | sed -n '2p'))"
      ;;
    *)
      warn "Claude browser capability: no active local browse binary, browser MCP, or browser plugin detected"
      ;;
  esac
}
