...
visibility_output="$(cd "$WORKDIR" && "$CODEX_BIN" exec --skip-git-repo-check \
  "Do not read any repository-specific files. First list the available globally visible skills in this session. Then reply with exactly one line: GLOBAL-SUPPORT-SKILLS-OK if you can see debug, tdd-execution, release-check, frontend-refactor, bugfix-verification, api-change-check, database-design, api-design, and architecture-design." \
  2>&1 || true)"
