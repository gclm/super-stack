...
assert_file "${HOME}/.super-stack/runtime/codex/AGENTS.md"
assert_file "${HOME}/.super-stack/runtime/codex/config.toml"
assert_dir "${HOME}/.super-stack/runtime/codex/agents"
assert_file "${HOME}/.super-stack/runtime/claude/CLAUDE.md"
...
assert_not_dir "${HOME}/.super-stack/runtime/scripts/install"
assert_not_dir "${HOME}/.super-stack/runtime/scripts/smoke"
assert_not_dir "${HOME}/.super-stack/runtime/scripts/test"
assert_not_dir "${HOME}/.super-stack/runtime/scripts/release"
assert_not_exists "${HOME}/.super-stack/runtime/scripts/lib/install-state.sh"

assert_contains "${HOME}/.codex/AGENTS.md" "Managed by super-stack; bootstrap only."
assert_contains "${HOME}/.codex/AGENTS.md" "${HOME}/.super-stack/runtime/codex/AGENTS.md"
assert_contains "${HOME}/.claude/CLAUDE.md" "Managed by super-stack; bootstrap only."
assert_contains "${HOME}/.claude/CLAUDE.md" "${HOME}/.super-stack/runtime/claude/CLAUDE.md"
...
runtime_browser_check="$(bash "${HOME}/.super-stack/runtime/scripts/check/check-browser-capability.sh")"
printf '%s\n' "$runtime_browser_check" | rg -q 'ACTIVE_MCP' || fail "runtime browser capability 探针未检测到活跃 MCP"

runtime_codex_check="$(bash "${HOME}/.super-stack/runtime/scripts/check/check-codex-runtime.sh")"
printf '%s\n' "$runtime_codex_check" | rg -q '== Codex 运行时检查 ==' || fail "runtime Codex 检查脚本未正常输出"

runtime_skill_validation="$(cd "${REPO_ROOT}" && python3 "${HOME}/.super-stack/runtime/scripts/check/validate-skills.py")"
printf '%s\n' "$runtime_skill_validation" | rg -q '校验通过：0 个错误' || fail "runtime skill 校验脚本未通过仓库校验"
